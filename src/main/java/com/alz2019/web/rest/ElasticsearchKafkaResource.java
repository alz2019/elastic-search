...
import com.alz2019.config.KafkaSseConsumer;
import com.alz2019.config.KafkaSseProducer;
...
@RestController
@RequestMapping("/api/elasticsearch-kafka")
public class ElasticsearchKafkaResource {

    private final Logger log = LoggerFactory.getLogger(ElasticsearchKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public ElasticsearchKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
